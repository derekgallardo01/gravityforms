...
//print_r($form);die();
		...
		switch ( RGForms::post( 'action' ) ) {
			...
			case 'bulk' :
				...
				if ( $_POST['bulk_action'] == 'archive' ) {
					...
					RGFormsModel::archive_notes( $_POST['note'] );
				}
				break;
			...
			foreach ( $notes as $note ) {
				...
				<tr valign="top" data-id="<?php echo esc_attr( $note->id ); ?>" data-type="<?php echo esc_attr( $note->note_type ); ?>"<?php echo rgobj( $note, 'sub_type' ) ? ' data-sub-type="' . esc_attr( $note->sub_type ) . '"' : ''; ?>>
					...
					<td class="entry-detail-note<?php echo $is_last ? ' lastrow' : '' ?>">
						...
						<div class="detail-note-content <?php echo implode( ' ', $classes ); ?>"><?php echo nl2br( ( $note->value ) ) ?></div>
					</td>
	...
	public static function meta_box_notes( $args, $metabox ) {
		...
		?>
		<label for="name"><a href="admin.php?page=gf_entries&view=<?php echo $_GET["view"];?>&id=<?php echo $_GET["id"];?>&lid=<?php echo $_GET["lid"];?>&dir=<?php echo $_GET["dir"];?>&filter&paged=<?php echo $_GET["paged"];?>&pos=<?php echo $_GET["pos"];?>&field_id&operator&archive_note=no"><?php esc_html_e( 'Notes', 'gravityforms' ); ?></a></label>
		<?php /*?>&nbsp;&nbsp;&nbsp;
		<label for="name"><?php esc_html_e( 'Archive Notes', 'gravityforms' ); ?></label><?php */?>
		<?php
		}else{?>
		...
		$notes_dir = (array)RGFormsModel::get_lead_notes_archive_dir($_GET["lid"]);
		
		?>
		<label for="name"><?php /*?><a href="admin.php?page=gf_entries&view=<?php echo $_GET["view"];?>&id=<?php echo $_GET["id"];?>&lid=<?php echo $_GET["lid"];?>&dir=<?php echo $_GET["dir"];?>&filter&paged=<?php echo $_GET["paged"];?>&pos=<?php echo $_GET["pos"];?>&field_id&operator&archive_note=yes"><?php esc_html_e( 'Archive Notes', 'gravityforms' ); ?></a><?php */?>
		<select name="archive_note" id="archive_note" onchange="location.href='admin.php?page=gf_entries&view=<?php echo (isset($_GET["view"])?$_GET["view"]:'');?>&id=<?php echo (isset($_GET["id"])?$_GET["id"]:'');?>&lid=<?php echo (isset($_GET["lid"])?$_GET["lid"]:'');?>&dir=<?php echo (isset($_GET["dir"])?$_GET["dir"]:'');?>&filter&paged=<?php echo (isset($_GET["paged"])?$_GET["paged"]:'');?>&pos=<?php echo (isset($_GET["pos"])?$_GET["pos"]:'');?>&field_id&operator&archive_note=yes&archive_dir='+this.value">
		<option value="">View All archive</option>
		<?php
		$dirarr = array();
		
		
		foreach($notes_dir as $nts_dir){
		
			$archive_dir_name = $nts_dir->archive_dir;
		
			if(!empty($archive_dir_name) && $archive_dir_name!='' && !in_array($archive_dir_name, $dirarr)){
			$dirarr[] = $archive_dir_name;
			
			?>                            
			<option value="<?php echo $archive_dir_name;?>" <?php if(isset($_GET["archive_dir"]) && $archive_dir_name == $_GET["archive_dir"] ){ ?> selected="selected" <?php } ?> ><?php echo ucfirst($archive_dir_name);?></option>
			<?php
			}
		}
		...
		<label for="name">
		<?php
		$gview = isset($_GET["view"])?$_GET["view"]:'';
		$gid = isset($_GET["id"])?$_GET["id"]:'';
		$glid = isset($_GET["lid"])?$_GET["lid"]:'';
		$gdir = isset($_GET["dir"])?$_GET["dir"]:'';
		$gpaged = isset($_GET["paged"])?$_GET["paged"]:'';
		$gpos = isset($_GET["pos"])?$_GET["pos"]:'';
		$garchive_note = isset($_GET["archive_note"])?$_GET["archive_note"]:'';
		$garchive_dir = isset($_GET["archive_dir"])?$_GET["archive_dir"]:'';
		
		
		?>
		<select name="archive_note" id="archive_note" onchange="location.href='admin.php?page=gf_entries&view=<?php echo $gview;?>&id=<?php echo $gid;?>&lid=<?php echo $glid;?>&dir=<?php echo $gdir;?>&filter&paged=<?php echo $gpaged;?>&pos=<?php echo $gpos;?>&field_id&operator&archive_note=<?php echo $garchive_note;?>&archive_dir=<?php echo $garchive_dir;?>&archive_year='+this.value">
		<option value="">View All Notes</option>
		<?php
		$n_year = date('Y')-10;
		for($ty = $n_year;$ty<=date('Y');$ty++){
		?>                            
		<option value="<?php echo $ty;?>" <?php if(isset($_GET["archive_year"]) && $ty == $_GET["archive_year"] ){ ?> selected="selected" <?php } ?> ><?php echo $ty;?></option>
		<?php
		}
		?>
		</select>
