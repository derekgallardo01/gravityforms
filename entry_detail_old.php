...
//print_r($form);die();
	...
	public static function meta_box_notes( $args, $metabox ) {
		...
		<select name="archive_note" id="archive_note" onchange="location.href='admin.php?page=gf_entries&view=<?php echo $_GET["view"];?>&id=<?php echo $_GET["id"];?>&lid=<?php echo $_GET["lid"];?>&dir=<?php echo $_GET["dir"];?>&filter&paged=<?php echo $_GET["paged"];?>&pos=<?php echo $_GET["pos"];?>&field_id&operator&archive_note=yes&archive_dir='+this.value">
		<option value="">View All archive</option>
		<?php
		$dirarr = array();
		$notes_dir = (array)RGFormsModel::get_lead_notes_archive_dir($lead["id"]);
		...
		foreach($notes_dir as $nts_dir){
			...
			?>                            
			<option value="<?php echo $archive_dir_name;?>" <?php if($archive_dir_name == $_GET["archive_dir"] ){ ?> selected="selected" <?php } ?> ><?php echo ucfirst($archive_dir_name);?></option>
			<?php
			}
		}
