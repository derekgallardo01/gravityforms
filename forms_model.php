...
class GFFormsModel {
	...
	public static function get_lead_notes_archive_dir($lead_id){
        global $wpdb;
        $notes_table = self::get_entry_notes_table_name();
        return $wpdb->get_results($wpdb->prepare("  SELECT n.id, n.user_id, n.date_created, n.value, n.archive, n.archive_dir FROM $notes_table n WHERE entry_id=%d and archive = 1 ORDER BY id", $lead_id));
    }
	...
	public static function get_lead_notes( $lead_id ) {
		...
		$dirsql .= " and date_format(n.date_created,'%Y') = '".$_GET["archive_year"]."' ";
			...
			$dirsql = " and (n.archive is null or n.archive = '') ";
		}
		$finalsql = $wpdb->prepare(
				"  SELECT n.id, n.user_id, n.date_created, n.value, n.note_type, n.sub_type, ifnull(u.display_name,n.user_name) as user_name, u.user_email
                                                    ...
                                                    WHERE entry_id=%d ", $lead_id
			);
		$finalsql = $finalsql." ".$dirsql." ORDER BY n.id ";
	...
	public static function get_field_value_long( $lead, $field_number, $form, $apply_filter = true ) {
		...
		$sql = $wpdb->prepare(
			" SELECT l.value FROM $detail_table_name d
                                ...
                                WHERE entry_id=%d AND field_number BETWEEN %s AND %s", $lead['id'], doubleval( $field_number ) - 0.0001, doubleval( $field_number ) + 0.0001
		);
